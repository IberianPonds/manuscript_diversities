...
gamma_mean_temp_2018 <- 
  ...
  geom_smooth(data = subset(div_env_df, scale=="gamma" & year=="2018" & group=="Bacteria"), 
              color = color_group[2], method = "lm", size = 1.25, formula = y~x) +

  #geom_point(data = subset(div_env_df, scale=="gamma" & year=="2018" & group=="Phytoplankton"), color = co#lor_group[3]) +
  ...
  #geom_point(data = subset(div_env_df, scale=="gamma" & year=="2018" & group=="Macroinvertebrates"), color = co#lor_group[5]) +
...
gamma_sd_temp_mean <- 
  ...
  ylim(0, 600) +
  xlim(1, 3) +
  scale_x_continuous(breaks = c(1.8, 2.0, 2.2)) +
  xlab("") +
  ylab(expression("Temporal average "*gamma*"-diversity")) +
  ggtitle("Regional Scale") +
  theme_tq() + 
...
alpha_mean_temp_mean <- 
  ...
  #geom_smooth(data = subset(div_env_df, scale == "alpha" & year == "Mean" & group == "Macroinvertebrates"),   #            aes(group = group == "Macroinvertebrates"), color = color_group[5],
  #            method = "lm", size = 1.25, formula = y~x) +
  ylim(0, 250) +
  xlim(2.33, 3.02) +
  scale_x_continuous(breaks = c(2.4, 2.6, 2.8, 3.0)) +
...
beta_mean_temp_2016 <- 
  ...
  geom_smooth(data = subset(div_env_df, scale == "beta2" & year == "2016" & group == "Mroinvertebrates"), 
             aes(group = group == "Macroinvertebrates"), color = color_group[5],
             method = "lm", size = 1.25, formula = formula) +
...
beta_sd_temp_mean <- 
  ...
  geom_point(data = subset(div_env_df, scale == "beta2" & year == "Mean" & group == "Macroinvertebrates"), color = color_group[5]) +
  geom_smooth(data = subset(div_env_df, scale == "beta2" & year == "Mean" & group == "Macroinvertebrates"),               aes(group = group == "Macroinvertebrates"), color = color_group[5],
              method = "lm", size = 1.25, formula = y~x) +
...
gamma_tmean <- annotate_figure(gamma_tmean, 
                               ...
                               bottom = text_grob(bquote(Log~T[Mean]~(ºC)), size = 14))
...
gamma_years_tmean <- ggarrange(gamma_mean_temp_2016 + rremove("ylab") + rremove("xlab"), 
                         ...
                         nrow = 1, ncol = 3, widths = c(1.25, 1, 1),
                         common.legend = T, legend = "bottom")
...
gamma_tsd <- annotate_figure(gamma_tsd, 
                             left = text_grob(expression("Temporal average  "*gamma*"-diversity"), rot = 90, vjust = 1, size = 16),
                               bottom = text_grob(bquote(Log~T[SD]~(ºC)), size = 14))
