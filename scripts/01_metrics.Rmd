---
title: "Multi-trophic spatial scaling across temperature gradient - data prep"
...
hybrid.geo.fit.st$group <- NA
hybrid.geo.fit.st <- hybrid.geo.fit.st %>%
  dplyr::select(taxa, domain, kingdom, phylum, class, order, family, genus, species, group, everything()) # reorganize columns


for (i in 1:nrow(hybrid.geo.fit.st)) { # loop species
  
  if (hybrid.geo.fit.st[i, "domain"] == "Bacteria" |
      hybrid.geo.fit.st[i, "kingdom"] == "Chromista" |
      hybrid.geo.fit.st[i, "kingdom"] == "Plantae" |
      hybrid.geo.fit.st[i, "kingdom"] == "Protozoa") {
    hybrid.geo.fit.st[i, "group"] <- "phyto" # set these to group phyto
    
  } else { # not phyto
    
    if (hybrid.geo.fit.st[i, "phylum"] == "Rotifera" |
        hybrid.geo.fit.st[i, "class"] == "Branchiopoda" |
        hybrid.geo.fit.st[i, "class"] == "Copepoda" |
        hybrid.geo.fit.st[i, "class"] == "Hexanauplia" |
        hybrid.geo.fit.st[i, "class"] == "Ostracoda") {
      hybrid.geo.fit.st[i, "group"] <- "zoo" # set these to group zoo
      
    } else { # not phyto or zoo
      
      if (hybrid.geo.fit.st[i, "phylum"] == "Mollusca" |
          hybrid.geo.fit.st[i, "phylum"] == "Nematoda" |
          hybrid.geo.fit.st[i, "phylum"] == "Annelida" |
          hybrid.geo.fit.st[i, "phylum"] == "Tardigrada" |
          hybrid.geo.fit.st[i, "phylum"] == "Gastrotricha" |
          hybrid.geo.fit.st[i, "phylum"] == "Nemertea" |
          hybrid.geo.fit.st[i, "phylum"] == "Platyhelminthes" |
          hybrid.geo.fit.st[i, "class"] == "Arachnida" |
          hybrid.geo.fit.st[i, "class"] == "Insecta" |
          hybrid.geo.fit.st[i, "class"] == "Malacostraca") {
        hybrid.geo.fit.st[i, "group"] <- "macro" # set these to group macro
      
        } else { # higher identification, not possible to assigned to a group
          hybrid.geo.fit.st[i, "group"] <- "not_assigned"
      }
    }
  }
}

# replace NOID or NA in "taxa" for correct taxon
hybrid.geo.fit.st$taxa[353] = "Miozoa"
hybrid.geo.fit.st$taxa[549] = "Miozoa"

# separate taxonomy from abundances
hybrid_tax <- hybrid.geo.fit.st[ , 1:11]
hybrid_abund <- hybrid.geo.fit.st[ , 12:ncol(hybrid.geo.fit.st)]
...
for (i in 1:nrow(hybrid_matrix2)) {
  hybrid_matrix2[, 353] <- sum(hybrid_matrix2[i, 353], hybrid_matrix2[i, 549])
}
hybrid_matrix2 <- hybrid_matrix2[ , -549]
...
grouping <- data.frame(rownames(hybrid.geo.fit.st), hybrid.geo.fit.st$group)  # select taxa and group columns
...
zoo_ <- zoo %>% select_if(function(x) !(all(is.na(x)) | all(x=="")))
...
MD2016 <- c("PN_16_01", "PN_16_03", "PN_16_05", "PN_16_07", "PN_16_09", "PN_16_11", "PN_16_13", "PN_16_16", "PN_16_19", "PN_16_20", "PN_16_23", "PN_16_24", "PN_16_25", "PN_16_27", "PN_16_28", "PN_16_30", "PN_16_31")
MD16 <- merged_matrix2[which(merged_matrix2$sample_code %in% MD2016), ] %>%
  arrange(sample_code)

MD2017 <- c("PN_17_01", "PN_17_03", "PN_17_05", "PN_17_07", "PN_17_09", "PN_17_11", "PN_17_13", "PN_17_16", "PN_17_19", "PN_17_20", "PN_17_23", "PN_17_24", "PN_17_25", "PN_17_27", "PN_17_28", "PN_17_30", "PN_17_31")
MD17 <- merged_matrix2[which(merged_matrix2$sample_code %in% MD2017), ] %>%
  arrange(sample_code)

MD2018 <- c("PN_18_01", "PN_18_03", "PN_18_05", "PN_18_07", "PN_18_09", "PN_18_11", "PN_18_13", "PN_18_16", "PN_18_19", "PN_18_20", "PN_18_23", "PN_18_24", "PN_18_25", "PN_18_27", "PN_18_28", "PN_18_30", "PN_18_31")
MD18 <- merged_matrix2[which(merged_matrix2$sample_code %in% MD2018), ] %>%
  arrange(sample_code)

MR2016 <- c("MR_16_01", "MR_16_05", "MR_16_06", "MR_16_07", "MR_16_09", "MR_16_11", "MR_16_13", "MR_16_14", "MR_16_15", "MR_16_17", "MR_16_19", "MR_16_21", "MR_16_23", "MR_16_25", "MR_16_27", "MR_16_29", "MR_16_31")
MR16 <- merged_matrix2[which(merged_matrix2$sample_code %in% MR2016), ] %>%
  arrange(sample_code)

MR2017 <- c("MR_17_01", "MR_17_05", "MR_17_06", "MR_17_07", "MR_17_09", "MR_17_11", "MR_17_13", "MR_17_14", "MR_17_15", "MR_17_17", "MR_17_19", "MR_17_21", "MR_17_23", "MR_17_25", "MR_17_27", "MR_17_29", "MR_17_31")
MR17 <- merged_matrix2[which(merged_matrix2$sample_code %in% MR2017), ] %>%
  arrange(sample_code)

MR2018 <- c("MR_18_01", "MR_18_05", "MR_18_06", "MR_18_07", "MR_18_09", "MR_18_11", "MR_18_13", "MR_18_14", "MR_18_15", "MR_18_17", "MR_18_19", "MR_18_21", "MR_18_23", "MR_18_25", "MR_18_27", "MR_18_29", "MR_18_31")
...
community_matrix <- merged_matrix3 %>% dplyr::select(., acari:xtant)
...
#fungi <- decostand(fungi, method = "pa")
...
#  abund_macro,
#  community_matrix,
#  global_pa_community_matrix,
#     
#     pa_community_matrix, log_community_matrix,
#     
#     file = "../inputs/consolidated_matrices.RData"
#     )
...
#
# Input:
#   comm_matrix      = community matrix (species × samples) with first 3 columns
#                      containing sample identifiers (sample_code, site, year)
#   factors          = metadata frame containing sample-level factors
#   environmental_data = environmental metadata for sample ordering
#   indices          = MOBR diversity metrics to calculate
#   effort           = standardized rarefaction size (Sn)
#
# Output: A list containing:
#   alpha_samples     = alpha-diversity per sample
#   beta_samples      = beta-diversity replicated per sample
#   gamma_samples     = gamma-diversity replicated per sample
#   alpha_site        = same as alpha_samples
#   beta_site         = site-level beta-diversity
#   gamma_site        = site-level gamma-diversity
#   full_table        = wide-format alpha, beta, gamma (numeric)
#   SBR               = raw MOBR output
#
# Notes:
#   - Sample order is always matched to environmental_data to ensure consistency.
#   - Alpha is stored per sample; beta and gamma are site-level but replicated
#     to samples where needed.
#   - The returned object is fully compatible with downstream merging and plotting.
# ==============================================================================
...
compute_mobr_div <- function(comm_matrix, factors, environmental_data,
  ...
  #### 3. Alpha diversity (sample-level) -------------------------------------
  ...
  #### 6. Final unified table (wide-format) ----------------------------------
...
```{r Save objects}
save(site_labels, group_labels, color_ib, color_group,
     rarefaction_df, div_env_df, log_ratio_df,
     file = "../inputs/3b_diversity_env_df_TEST.RData"
     )
```
